...
First of all, I have absolutely no experience with utility CSS classes (or atomic CSS). I only have experience with UI component libraries, which have anything in store for me. Like, `Button`, `Card`, `Dropdown`, are provided and ready-to-use.
...
The first thing people always say to me about Tailwind CSS is how easy it is to pour the styling ideas you have in your head into the code. And after I gave it a try, it turned out to be true but with conditions.

You must be familiar with Tailwind CSS utility class names. For example, if we want to center everything inside a div using flex in vanilla CSS, we write it like this:
...
The second thing is the performance. Because Tailwind CSS uses a library called PurgeCSS to remove unused styles from our project. That way, the compiled CSS size becomes tiny and friendly to users with slow connections.

How to make the PurgeCSS works, is by adding `purge` config list to `tailwind.config.js`. For example:
...
So just-in-time feature is basically a new compiler that generates styles on-demand as you author your templates instead of generating everything in advance at initial build time. So initial development builds time becomes noticeably faster.
...
You can use Tailwind CSS just-in-time feature by adding `mode` key to the `tailwind.config.js` like this:
